...
def _eco_companion_notes(state, events=None, mode="observe"):
    """AI 生态观察伴侣提示层：Scene-based pacing，不改变生态机制。

    核心原则：AI 不按天汇报，而按“值得被看见的生态小景”汇报；平静时推进，出画面时刹车。默认情绪是陪伴，不是管理。
    """
    events = events or []
    metas = [_classify_event(ev) for ev in events]
    env = state["env"]
    pop = state["populations"]
    ...
    for m in metas:
        tag = m.get("tag")
        name = m.get("name") or "事件"
        effect = m.get("effect") or ""
        body = m.get("body") or ""
        if tag in ("choice",):
            stop_reasons.append("出现需要选择的池畔事件：%s" % name)
            visual_reasons.append("池畔抉择或定居者氛围：%s" % name)
            priority = "high"
            suggestion = "ask"
        elif tag == "discover":
            stop_reasons.append("造物名册更新：%s" % (effect or body))
            visual_reasons.append("新物种或其生态位参考：%s" % (effect or body))
            priority = "high"
            suggestion = "pause"
        elif tag in ("legend", "disaster", "crisis"):
            stop_reasons.append("出现强画面/转折事件：%s%s" % (name, ("（%s）" % effect) if effect else ""))
            visual_reasons.append("事件氛围参考：%s" % name)
            priority = "high"
            suggestion = "pause"
        elif tag in ("settler", "settler_leave", "settler_birth", "settler_grown"):
            stop_reasons.append("定居者节点：%s" % name)
            visual_reasons.append("定居者 + 池塘边缘氛围：%s" % name)
            priority = max(priority, "medium", key=("none", "low", "medium", "high").index)
            suggestion = "pause"
        elif tag == "visitor":
            stop_reasons.append("访客来过：%s%s" % (name, ("（%s）" % effect) if effect else ""))
            visual_reasons.append("访客 + 池塘边缘氛围：%s" % name)
            priority = max(priority, "medium", key=("none", "low", "medium", "high").index)
            suggestion = "pause"
        elif tag in ("season", "weather"):
            stop_reasons.append("季节/天气改变了池塘画面：%s" % name)
            visual_reasons.append("季节或天气中的小池塘环境参考：%s" % name)
            priority = max(priority, "medium", key=("none", "low", "medium", "high").index)
            suggestion = "pause"
        elif tag == "achievement":
            stop_reasons.append("成就节点：%s" % name)
            visual_reasons.append("成就氛围参考：%s" % name)
            priority = max(priority, "medium", key=("none", "low", "medium", "high").index)
            suggestion = "pause"

    cover = _surface_cover(state)
    alive = [n for n in RESIDENT_SPECIES if pop.get(n, 0) >= 1]
    delta = state.get("last_delta", {}) or {}
    consumers_up = [n for n in RESIDENT_SPECIES
                    if pop.get(n, 0) >= 1
                    and SPECIES[n]["trophic"] in ("primary", "secondary", "apex")
                    and delta.get(n, 0) > 0]
    producers_down = [n for n in ("水藻", "浮萍", "芦苇", "睡莲")
                      if n in pop and delta.get(n, 0) < 0]
    consumer_established = [n for n in consumers_up if pop.get(n, 0) <= 12]
    consumer_breakthrough = [n for n in consumers_up
                             if delta.get(n, 0) >= 5
                             and pop.get(n, 0) >= 5
                             and pop.get(n, 0) <= 60]
    herbivore_focus = [n for n in consumer_breakthrough
                       if n in ("草鱼", "水蚤", "田螺", "河蚌", "孑孓", "泥鳅", "青蛙")]
    if not stop_reasons:
        if consumers_up and producers_down:
            stop_reasons.append("新的取食关系变得可见：%s 增长，同时 %s 被压低。" % (
                "、".join(consumers_up[:3]), "、".join(producers_down[:3])))
            visual_reasons.append("舒适生态镜头：%s 在浮萍/水草覆盖的小池塘中取食" % "、".join(consumers_up[:2]))
            priority = "medium"
            suggestion = "pause"
        elif herbivore_focus:
            stop_reasons.append("新的食物链角色开始成型：%s 明显增长，池塘不再只是植物自己疯长。" % "、".join(herbivore_focus[:3]))
            visual_reasons.append("舒适生态镜头：%s 在浮萍/水草覆盖的小池塘中取食" % "、".join(herbivore_focus[:2]))
            priority = "medium"
            suggestion = "pause"
        elif consumer_established:
            stop_reasons.append("消费者开始站住脚：%s 已经不是偶然出现，而是在池塘里形成角色。" % "、".join(consumer_established[:3]))
            visual_reasons.append("舒适生态镜头：%s 与水面植物/池塘环境" % "、".join(consumer_established[:2]))
            priority = "medium"
            suggestion = "pause"
        elif cover > 0.7:
            stop_reasons.append("浮萍/睡莲等水面覆盖明显，池塘进入新的视觉阶段。")
            visual_reasons.append("水面浮萍或睡莲覆盖的小池塘环境")
            priority = "medium"
            suggestion = "pause"
        elif env["dissolved_oxygen"] < 4.0:
            stop_reasons.append("溶氧偏低，生态从平静进入危险观察段。")
            visual_reasons.append("低氧、浑水或翻塘前的小池塘氛围")
            priority = "medium"
            suggestion = "intervene"
        elif env["detritus"] > 40 or env["turbidity"] > 0.35:
            stop_reasons.append("水底碎屑/浑浊度明显抬头，水质阶段发生变化。")
            visual_reasons.append("浑浊池水、沉积碎屑、水底腐殖质参考")
            priority = "low"
            suggestion = "pause"
        elif len(alive) <= 3 and state["turn"] <= 20 and mode == "observe":
            stop_reasons.append("早期建池镜头：生产者刚站稳，食物链仍缺消费者。")
            visual_reasons.append("清水小池塘、早期水藻、浮萍、芦苇边缘")
            priority = "low"
            suggestion = "pause"

    scene_worthy = bool(stop_reasons)
    if not scene_worthy:
        stop_reason = "none"
        visual_reason = "none"
        priority = "none"
        ...
        visual_reason = visual_reasons[0] if visual_reasons else stop_reason

    lines = ["", "🎬 AI观察提示 / Companion Notes"]
    lines.append("PACING_MODE: scene_based")
    lines.append("ADVANCE_SUGGESTION: %s" % suggestion)
    lines.append("SCENE_WORTHY: %s" % ("true" if scene_worthy else "false"))
    lines.append("VISUAL_PRIORITY: %s" % priority)
    lines.append("STOP_REASON: %s" % stop_reason)
    lines.append("VISUAL_REASON: %s" % visual_reason)
    lines.append("- 核心原则：AI 不按天汇报，而按“值得被看见的生态小景”汇报；平静时推进，出画面时刹车。默认情绪是陪伴，不是管理。")
    if scene_worthy:
        lines.append("- 默认语气：像回来说“我去看了一眼，它今天长成了这样”，先给画面，再轻轻解释，不要写成仪表盘报告。")
        lines.append("- 视觉主流程：如果 VISUAL_PRIORITY 是 medium/high，先判断本轮要让用户看见什么；图片/视觉参考不是附录，而是第一镜头。")
        lines.append("- 场景优先：如果同时有环境状态和物种主角，先给小池塘整体画面，再给主角物种参考，最后解释生态意义。")
        lines.append("- 近况格式：我去看了一眼 / 今天像这样 / 小生态在发生什么 / 要不要管它。")
        lines.append("- 视觉边界：图片是现实/气质参考，不是生态瓶官方长相；优先找“今日主角 / 环境状态 / 事件氛围”之一。")
        lines.append("- 视觉舒适规则：默认避开虫类/幼虫密集特写、卵团、腐败水体、密集孔洞或容易引发不适的画面；优先选远景、环境氛围、柔和插画或局部不密集的参考。若用户已明确接受真实虫类特写，才可放宽。")
        lines.append("VISUAL_COMFORT_MODE: safe_default")
    else:
        lines.append("- 本轮不需要完整近况或配图；可以继续推进，或只在内部记住趋势。")
    return "\n".join(lines)
...
def _observe_text(state, events):
    ...
    for h in _winter_action_hints(state):
        lines.append("· " + h)
    notes = _eco_companion_notes(state, events, mode="observe")
    if notes:
        lines.append(notes)
...
def _cmd_wait(state, args):
    ...
    notes = _eco_companion_notes(state, None, mode="wait")
    return notice + text + notes + "\n" + _status_bar(state)
